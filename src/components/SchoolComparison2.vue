<template>
  <div class="comparison-container">
    ...
    <div v-if="currentDisciplineData" class="tables-container">
      ...
      <div v-else-if="activeTab !== 'benchmark' && dynamicIndicatorData[currentlySelectedDisciplineCode]?.[activeTab]" class="tables-container">
        <el-card
          v-for="(tableContent, tableName) in dynamicIndicatorData[currentlySelectedDisciplineCode][activeTab]"
          :key="tableName"
          shadow="never"
          class="data-card"
        >
...
const dynamicIndicatorData = ref({}); // 动态指标数据
...
const handleTabClick = async (tab) => {
  ...
  activeTab.value = tab.props.name; // 更新 activeTab

  // 如果不是 'all' 或 'benchmark'，并且有选中的学科，则尝试获取动态指标数据
  if (activeTab.value !== 'all' && activeTab.value !== 'benchmark' && currentlySelectedDisciplineCode.value) {
    const discipline = currentlySelectedDisciplineCode.value;
    const school = activeTab.value; // 标签页的名字就是学校名称

    // 检查是否已经加载过数据，避免重复请求
    if (!dynamicIndicatorData.value[discipline]?.[school]) {
      await fetchDynamicIndicators(discipline, school);
    }
  }
};
...
const handleCellClick = async (row, column) => {
  if (!currentlySelectedDisciplineCode.value) {
    console.warn('当前未选择学科。无法获取详细数据。');
    return;
  }

  const school = column.label;   // 根据您的要求，列的标签是学校名称
  const indicator = row[0];     // 根据您的要求，行的第一个元素是指标名称
  const discipline = currentlySelectedDisciplineCode.value;

  // 避免点击非数据列触发详情，例如第一列的类别和第二列的指标名称列
  // 在 'all' 标签页中，数据列通常从索引 2 开始（假设索引 0 是类别，索引 1 是指标名称）
  const columnIndex = column.no; // Element Plus 提供 'no' 属性来获取列的索引
  if (activeTab.value === 'all' && (columnIndex === 0 || columnIndex === 1)) {
    console.log('点击了非数据列，不显示详情。');
    return;
  }

  detailDialogTitle.value = `${school} - ${indicator} 详情`;

  // 调用新函数获取详细数据
  const dataFromApi = await fetchSpecificIndicatorData(discipline, school, indicator);

  if (dataFromApi && dataFromApi.tableData && dataFromApi.tableHeaders) {
    detailTableData.value = dataFromApi.tableData;
    detailTableHeaders.value = dataFromApi.tableHeaders;
    showDetailDialog.value = true;
  } else {
    console.warn(`未找到 '${school}' - '${indicator}' 的详细数据或数据格式不正确。`);
    detailTableData.value = [];
    detailTableHeaders.value = [];
    detailDialogTitle.value = '无详细数据'; // 即使没有数据也打开弹窗提示
    showDetailDialog.value = true;
  }
};

const fetchDynamicIndicators = async (discipline, school) => {
  if (!discipline || !school) {
    console.warn('获取动态指标数据时，学科或学校参数缺失。');
    return;
  }
  console.log(`正在获取 ${discipline} - ${school} 的动态指标数据...`);
  try {
    const response = await api.get('/discipline/school-all-indicators', {
      params: {
    ...
    if (response.data?.code === 200 && response.data?.data ) {
      // 确保嵌套结构存在
      if (!dynamicIndicatorData.value[discipline]) {
        dynamicIndicatorData.value[discipline] = {};
      }
      dynamicIndicatorData.value[discipline][school] = response.data.data.schoolIndicators[school];
      console.log(`成功获取 ${discipline} - ${school} 的动态指标数据:`, JSON.stringify(response.data.data.schoolIndicators[school]));
    } else {
      console.warn(`获取 ${discipline} - ${school} 的动态指标数据时，返回的数据格式不正确或为空，或code不为200。`);
      // 可以设置为 null 或空对象，表示数据获取失败
      if (!dynamicIndicatorData.value[discipline]) {
        dynamicIndicatorData.value[discipline] = {};
      }
      dynamicIndicatorData.value[discipline][school] = null;
    }
  } catch (error) {
    console.error(`获取 ${discipline} - ${school} 的动态指标数据失败:`, error);
    if (!dynamicIndicatorData.value[discipline]) {
      dynamicIndicatorData.value[discipline] = {};
    }
    dynamicIndicatorData.value[discipline][school] = null;
...
const fetchSpecificIndicatorData = async (discipline, school, indicator) => {
  if (!discipline || !school || !indicator) {
    console.warn('获取特定指标数据时，学科、学校或指标参数缺失。');
    return null;
  }

  try {
    const response = await api.get('/discipline/school-specific-indicator', {
      params: {
        discipline: discipline,
        school: school,
        indicator: indicator
    ...
    if (response.data?.code === 200 && response.data?.data) {
      console.log(`成功获取 ${discipline} - ${school} - ${indicator} 的详细数据:`, response.data.data);
      // 期望新接口直接返回一个包含 tableData 和 tableHeaders 的对象
      return response.data.data;
    } else {
      console.warn(`获取 ${discipline} - ${school} - ${indicator} 的详细数据时，返回的数据格式不正确或为空，或code不为200。`);
      return null;
    }
  } catch (error) {
    console.error(`获取 ${discipline} - ${school} - ${indicator} 的详细数据失败:`, error);
    return null;
  }
};
